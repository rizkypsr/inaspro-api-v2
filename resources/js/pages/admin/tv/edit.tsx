...
export default function EditTv({ tv, tvCategories }: Props) {
  const [imagePreview, setImagePreview] = useState<string | null>(
    tv.image ? (tv.image.startsWith('http') ? tv.image : `/storage/${tv.image}`) : null
  );
  const [removeCurrentImage, setRemoveCurrentImage] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const { data, setData, post, processing, errors } = useForm({
    tv_category_id: tv.tv_category_id.toString(),
    title: tv.title,
    link: tv.link,
    ...
    <AppLayout>
      ...
      <div className="space-y-6 p-4">
        ...
        <div className="flex items-center space-x-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <span className="text-foreground">Edit {tv.title}</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Edit TV</h1>
            <p className="text-muted-foreground">
              Update TV channel information
            </p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Update the details for this TV channel
            </CardDescription>
          </CardHeader>
          <CardContent>
